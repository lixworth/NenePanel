...
// ------------------------------
		...
		for ($i=0; $i<sizeof($list); $i++) {
			...
			$target_relative = substr(realpath($source), strlen(realpath($tempdir)));
			$target = $archivesArray[$archive_nr]["targetdirectory"].$target_relative;//$net2ftp_globals["directory"] . $target_relative;
			$ftpmode = ftpAsciiBinary($source);
...
// If this fails, create the required directories and try again
			elseif (is_file($source) == true) {
				...
				if ($net2ftp_result["success"] == true) { $net2ftp_output["ftp_unziptransferfiles"][] = __("Copied file %1\$s", $target); }
					...
					$directory_to_create = $archivesArray[$archive_nr]["targetdirectory"];//$net2ftp_globals["directory"];
...
// Global variables
	...
	for ($i=1; $i<=sizeof($list); $i=$i+1) {
		if (isset($list[$i]["dirorfile"]) == true && isset($list[$i]["dirfilename"]) == true) {
			...
			$net2ftp_globals["state"] == "_unzip" || $net2ftp_globals["state"] == "view" || 
			$net2ftp_globals["state"] == "_zip" || 
