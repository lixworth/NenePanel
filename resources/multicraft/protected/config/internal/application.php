...
if (!@$config['timeout'])
    $config['timeout'] = 5;
if (!isset($config['use_pluginlist']))
    $config['use_pluginlist'] = @$config['use_bukget'];
...
return array(
    ...
    'components'=>array(
        ...
        'request'=>array(
            'enableCookieValidation'=>isset($config['enable_cookie_validation']) ? $config['enable_cookie_validation'] : true,
            'enableCsrfValidation'=>isset($config['enable_csrf_validation']) ? $config['enable_csrf_validation'] : true,
            'csrfCookie'=>array(
                'httpOnly'=>true,
            ),
        ),
        ...
        'cache'=>array(
            'class'=>((@$config['sqlitecache_schema'] && extension_loaded('pdo_sqlite')) ? 'CDbCache' : 'CDummyCache'),
        ),
        'mail'=>array(
