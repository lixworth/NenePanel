...
class CHtml
{
	...
	{
		if (in_array($tag, array('input', 'select', 'textarea')) && (!isset($htmlOptions['class']) || $htmlOptions['class'] == CHtml::$errorCss))
		{
			if ($tag != 'input' || in_array(@$htmlOptions['type'], array('text', 'file', 'button', 'password')))
				$htmlOptions['class'] = (isset($htmlOptions['class']) ? $htmlOptions['class'].' ' : '').'form-control';
		}
	 ...
	 */
	public static function css($text,$media='',$id='')
	{
		if($media!=='')
			$media=' media="'.$media.'"';
		return "<style type=\"text/css\"{$media}".($id ? ' id="_css_'.$id.'"' : '').">\n/*<![CDATA[*/\n{$text}\n/*]]>*/\n</style>";
	}
